...
""" Visualizing the performance of the Logistic Regression model by plotting
    ...
    Note: The predicted classes for each point in the 2D plotting space are 
          obtained using the same 126-dimensional Logistic Regression model used
          to predict the node classes. The points in the PCA-reduced 2D space
          are reprojected to the 126D space using PCA's inverse transform, and
          then their predicted classes are obtained from the 126-dimensional 
          ...
          loss from projecting them from 126 to 2 dimensions.
"""
...
grarep_logreg = grarep_labels["Logreg model"]
X_test = grarep_labels["Test embeddings"]
y_test = grarep_labels["Actual labels (test)"]

standard_scaler = grarep_logreg.named_steps["scaler"]
...
full_dim_grid = pca_model.inverse_transform(np.c_[xx.ravel(), yy.ravel()])

# Predict class labels for each point in the meshgrid
meshgrid_labels = grarep_logreg.predict(full_dim_grid)
...
# Compute percentiles to exclude extreme outliers
x_2, x_98 = np.percentile(pca_embeddings[:, 0], [2, 98])
y_2, y_98 = np.percentile(pca_embeddings[:, 1], [2, 98])
...
output_path = os.path.join(current_dir, "grarep_pca_fulldim.png")
